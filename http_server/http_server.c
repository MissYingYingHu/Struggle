...
int ResponseStaticFile(int sock,const char* file_path)
{
...
//    const char* body = "<head><meta http-equiv=\"content-type\" content=\"text/html;charset=utf-8\"></head>";
    ...
    send(sock,black_line,strlen(black_line),0);
    int fd = open("./root/404.html",O_RDONLY);
    if(fd < 0)
    {
        return;
    }
    sendfile(sock,fd,NULL,GetFileSize("./root/404.html"));
    send(sock,body,GetFileSize("./root/404.html"),0);
}
