...
int main()
{
  ...
  Date d1;
  int days;
  sscanf(buf,"year=%d&month=%d&day=%d&days=%d",&d1._year,&d1._month,&d1._day,&days);
  if(d1.IsInvalid() == false)
  {
  ...
  {
      d1 -= days;
      printf("<html><meta http-equiv=\"content-type\" content=\"text/html;charset=utf-8\"><h1>result is :%d年%d月%d日</h1></html>",d1._year,d1._month,d1._day);
  }
  d1 -= days;
//  fprintf(stderr,"days=%d",days);
//  fprintf(stderr,"year=%dmonth=%dday=%d\n",d1._year,d1._month,d1._day);
  return 0;
}
